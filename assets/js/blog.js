...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteModalElement = document.getElementById('modalEliminarPost');
    if (deleteModalElement) {
        const deleteModal = new bootstrap.Modal(deleteModalElement);
        const postInfoSpan = document.getElementById('delete-post-info');
        const deleteForm = document.getElementById('delete-post-form');
        const csrfTokenInput = document.getElementById('delete-csrf-token-post');
        
        document.body.addEventListener('click', (event) => {
            const deleteButton = event.target.closest('.delete-post-btn');
            if (deleteButton) {
                if (postInfoSpan) postInfoSpan.innerHTML = deleteButton.dataset.postInfo;
        ...
        deleteForm.addEventListener('submit', (event) => {
            ...
            handleFormSubmit(deleteForm, (data) => {
                const urlParts = deleteForm.action.split('/');
    ...
    document.body.addEventListener('submit', (event) => {
        ...
        const successCallback = (data) => {
            const isCreating = form.id === 'form-registrar-post';
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = `<table><tbody>${data.newRowHtml}</tbody></table>`;
            const newRow = tempDiv.querySelector('tr');

            if (isCreating) {
                document.getElementById('no-posts-row')?.remove();
                tableBody.appendChild(newRow);
                document.getElementById('modals-blog-container').insertAdjacentHTML('beforeend', data.newEditModalHtml);
            } else {
                document.getElementById(`post-row-${data.entidadId}`)?.replaceWith(newRow);
            }
