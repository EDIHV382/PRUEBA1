...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deleteModalElement = document.getElementById('modalEliminarPaciente');
    if (deleteModalElement) {
        const deleteModal = new bootstrap.Modal(deleteModalElement);
        const patientNameSpan = document.getElementById('delete-patient-name');
        const deleteForm = document.getElementById('delete-patient-form');
        const csrfTokenInput = document.getElementById('delete-csrf-token');
        document.body.addEventListener('click', (event) => {
            const deleteButton = event.target.closest('.delete-btn');
            if (deleteButton) {
                if (patientNameSpan) patientNameSpan.textContent = deleteButton.dataset.patientName;
    ...
    document.body.addEventListener('submit', (event) => {
        ...
        if (!form || !form.closest('.modal') || form.id === 'delete-patient-form') return;
        event.preventDefault();

        const successCallback = (data) => {
            const isCreating = form.id === 'form-registrar-paciente';
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = `<table><tbody>${data.newRowHtml}</tbody></table>`;
            const newRow = tempDiv.querySelector('tr');

            if (isCreating) {
                document.getElementById('no-pacientes-row')?.remove();
                tableBody.appendChild(newRow);
                document.getElementById('modals-pacientes-container').insertAdjacentHTML('beforeend', data.newEditModalHtml);
            } else {
                document.getElementById(`paciente-row-${data.entidadId}`)?.replaceWith(newRow);
            }
    ...
    if (buscador && tableBody) {
        buscador.addEventListener('input', e => {
            ...
            filas.forEach(fila => {
                if (fila.id === 'no-pacientes-row') return;
